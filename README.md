...
- [DSH k8e Sandbox](https://github.com/xiaods/k8e) - **484 stars** | `Apache-2.0`. A DeepSeek Harness sandbox bundle that routes filesystem and subprocess services through the k8e sandbox runtime.
...
- [DSH DeepSeek Web Login](https://github.com/cv-superding/dsh-deepseek-web-login) - **144 stars** | `Apache-2.0`. Use chat.deepseek.com web models as a DSH language model provider.
...
- [DSH Filesnap](https://github.com/extracurricular-ai/dsh-filesnap) - **42 stars** | `Apache-2.0`. A DeepSeek Harness rewind bundle that restores conversation state and changed files without requiring a Git repository.
  - Install: `dsh plugin --profile web add dsh-filesnap`

- [DSH Win32](https://github.com/sjh9714/dsh-win32) - **41 stars** | `MIT`. A Windows-focused DSH bundle with PowerShell execution, preset management, and sandbox-aware runtime checks.
...
- [DSH Better Edit](https://github.com/Rianico/dsh-better-edit) - **33 stars** | `MIT`. A DeepSeek Harness bundle that provides hash-addressed read and edit tools for verified file changes.
  - Install: `npx @deepseek-ai/dsh plugin --profile web add dsh-better-edit`

- [DeepSeek Harness ACP](https://github.com/openma-ai/deepseek-harness-acp) - **31 stars** | `Apache-2.0`. A DeepSeek Harness bundle that exposes sessions, tools, skills, and persistence through the Agent Client Protocol.
  - Install: `dsh plugin --profile acp add @openma/deepseek-harness-acp@latest`

- [DSH ChatGPT Subscription](https://github.com/Aa728848/dsh-chatgpt-subscription) - **31 stars** | `MIT`. Connect a ChatGPT subscription as a model provider in DSH.
  - Install: `dsh plugin --profile web add @eddyskywalker/dsh-chatgpt-subscription`

- [DSH OpenCode Go](https://github.com/Duskriver/dsh-opencode-go) - **31 stars** | `MIT`. Connect OpenCode Go models to the DSH model provider interface.
...
- [DSH Harness Remote](https://github.com/liguobao/ds-harness-remote) - **121 stars** | `MIT`. A DeepSeek Harness bundle that adds encrypted remote access for continuing sessions from desktop, Web, and Android clients.
...
- [DSH Prompt Optimizer](https://github.com/WestFox-AwA/dsh-prompt-optimizer) - **55 stars** | `BSD-3-Clause`. A DeepSeek Harness bundle that analyzes and rewrites prompts with configurable optimization steps before model requests.
...
- [DSH Tether](https://github.com/zexadev/dsh-tether) - **43 stars** | `MIT`. A DeepSeek Harness remote-access bundle that connects to Android and iOS clients over a peer-to-peer iroh transport.
...
- [DSH Web Startup Auth](https://github.com/GDWhisper/dsh-web-startup-auth) - **38 stars** | `MIT`. A DSH Web bundle that adds authenticated remote startup and configurable access controls for the local server.
...
- [DSH Remote Suite](https://github.com/Blank-not-black/dsh-Remote) - **33 stars** | `MIT`. A DSH remote-access suite with a profile plugin, gateway, mobile client, and Web management panel.
  - Install: `dsh plugin --profile web add dsh-remote-plugin`

- [DSH Computer Use macOS](https://github.com/988hj7tczd-oss/dsh-computer-use) - **32 stars** | `MIT`. A macOS desktop-control DSH bundle with scoped screen observation, input actions, app launching, and safety checks.
  - Install: `./install.sh`
...
- [DSH Mobile Gateway](https://github.com/Clarklevis1995/dsh-plugin-mobile-gateway) - **31 stars** | `MIT`. A DeepSeek Harness Web bundle that provides authenticated mobile gateway access with session control, events, files, and device pairing.
...
- [DSH Web LAN Access](https://github.com/AcidGr/dsh-web-lan-access) - **31 stars** | `MIT`. A DSH Web bundle that exposes the service on a local network with dynamic LAN and VPN trust handling.
...
- [ClearAI DSH](https://github.com/Clearailhc/clearai-dsh) - **508 stars** | `Apache-2.0`. Add an epistemic loop and local knowledge exploration to DeepSeek Harness.
...
- [Bailian Knowledge DSH](https://github.com/modelstudioai/cli) - **331 stars** | `Apache-2.0`. A DeepSeek Harness bundle that connects agents to Aliyun Model Studio knowledge bases through search and grounded Q&A tools.
...
- [DSH Project Brain](https://github.com/yj-liuzepeng/dsh-project-brain) - **77 stars** | `MIT`. Store project architecture and cross-session context for DSH agents.
...
- [DSH Mneme](https://github.com/slow-stack/mneme) - **63 stars** | `MIT`. A DSH memory plugin for persistent project knowledge and recall across sessions.
...
- [DSH Model Context Catalog](https://github.com/HOWILLMAKEIT/dsh-model-context-catalog) - **35 stars** | `MIT`. A DSH plugin that catalogs model context limits and displays provider metadata in a Web settings panel.
  - Install: `dsh plugin --profile web add dsh-model-context-catalog`

- [DSH Fund Research](https://github.com/PerryLink/dsh-fund-research) - **34 stars** | `Apache-2.0`. A DeepSeek Harness bundle for producing traceable fund research reports from sealed market snapshots.
...
- [StrataGate DSH Memory](https://github.com/diqierjia/StrataGate-AgentMemory) - **34 stars** | `MIT`. A DeepSeek Harness memory bundle for capturing sessions, building evidence-linked local memory, and reviewing recalled context.
  - Install: `dsh plugin --profile web add stratagate-dsh`

- [DSH Evolve in Git](https://github.com/Kytolly/dsh-evolve-in-git) - **31 stars** | `MIT`. A DSH bundle for Git-backed memory, skill evolution, privacy filtering, and repository-aware workflow tools.
  - Install: `dsh plugin --profile web add github:Kytolly/dsh-evolve-in-git`

- [DSH Knowledge](https://github.com/Soren-ABT/dsh-knowledge) - **31 stars** | `AGPL-3.0`. A DSH knowledge bundle for document management, chunking, embeddings, retrieval, and a Web administration panel.
...
- [DSH Memoir](https://github.com/Qinling-Melon-Farmers/dsh-memoir) - **30 stars** | `Apache-2.0`. Keep local project memory available across DSH sessions.
...
- [DSH Token Pet](https://github.com/Jimmy0123-ux/dsh-token-pet) - **55 stars** | `MIT`. A DSH Web companion that displays token usage and task status as a configurable animated pet.
...
- [Denia DSH Skin](https://github.com/Ewnscat-ya/dsh-client-ui-skin-denia) - **36 stars** | `CC-BY-NC-SA-4.0`. A DSH Web skin bundle with the Denia visual theme and a configurable client roster entry.
...
- [DSH OpenCode Palette](https://github.com/FeatherHunter/dsh-opencode-palette) - **33 stars** | `MIT`. A DeepSeek Harness Web bundle that adds 38 OpenCode-inspired color themes with one-click switching.
  - Install: `dsh plugin --profile web add dsh-opencode-palette`

- [DSH Any Background](https://github.com/Tkingxiao/dsh-any-background) - **32 stars** | `MIT`. A DeepSeek Harness Web appearance bundle for theme colors, image or video wallpapers, and per-surface opacity and blur.
  - Install: `dsh plugin --profile web add dsh-any-background`

- [DSH Bloom Theme](https://github.com/webkubor/dsh-bloom-theme) - **32 stars** | `MIT`. A DeepSeek Harness Web theme bundle with Bloom palettes, configurable appearance settings, and a live client skin.
  - Install: `dsh plugin add @kubor/dsh-bloom-theme`

- [DSH UI Skin Theme](https://github.com/chouxiaohuai/dsh-uiskin-theme) - **32 stars** | `No standard license`. A DeepSeek Harness Web bundle that adds an ocean-themed interface, light and dark switching, and live thinking-time display.
  - Install: `dsh plugin --profile web add github:chouxiaohuai/dsh-uiskin-theme#main`

- [Catppuccin DSH Theme](https://github.com/NoNameLeGo/dsh-catppuccin-theme) - **31 stars** | `MIT`. A DeepSeek Harness Web theme plugin with Latte, Frappé, Macchiato, and Mocha palettes plus optional glass effects.
  - Install: `dsh plugin --profile web add @nonamelego/dsh-catppuccin`

- [DSH Kimino Theme](https://github.com/niiang/dsh-kimino-theme) - **31 stars** | `MIT`. A DeepSeek Harness Web theme inspired by Kimi no Na wa with switchable visual styles that restore cleanly when removed.
...
- [DSH Plugin Shop](https://github.com/LivXue/dsh-plugin-shop) - **854 stars** | `Apache-2.0`. A DeepSeek Harness Web bundle for browsing, installing, enabling, and updating community plugins from a settings panel.
...
- [EchoCat Skill Panel](https://github.com/VDERR/dsh-echocat-skill-panel) - **201 stars** | `MIT`. Audit skill calls and manage installed skills from the DSH interface.
  - Install: `dsh plugin --profile web add dsh-echocat-skill-panel`

- [DSH Plugins Marketplace](https://github.com/bradeGithub/DSH-Plugins-Marketplace) - **164 stars** | `MIT`. A Web plugin market for discovering, installing, updating, and removing community DSH plugins from the settings interface.
...
- [DSH Plugin Hub Console](https://github.com/Noob-stupid/dsh-plugin-gating-hub) - **88 stars** | `MIT`. A DeepSeek Harness Web bundle for viewing installed plugins, toggling profile rows, checking compatibility, and finding installable packages.
...
- [DSH Watcher](https://github.com/aa2246740/dsh-watcher) - **68 stars** | `MIT`. A DSH Web plugin that shows live working activity, tool progress, and turn summaries in the conversation interface.
...
- [DSH Better Display](https://github.com/aa2246740/dsh-better-display) - **54 stars** | `MIT`. A DeepSeek Harness Web bundle that adds improved message rendering, session display controls, and related conversation UI features.
...
- [DSH Raw HTML v2](https://github.com/plolpl789/dsh-raw-html-v2) - **42 stars** | `MIT`. A DSH Web bundle that renders raw HTML documents in a sandboxed panel for agent-generated output.
...
- [DSH Plugins Collection](https://github.com/sjtuszh/dsh-plugins) - **40 stars** | `MIT`. A collection of DeepSeek Harness Web bundles for cost panels, file actions, and related interface utilities.
...
- [DSH Session Workbench](https://github.com/PolinniZhong/dsh-session-workbench) - **36 stars** | `MIT`. A DeepSeek Harness Web bundle for searching sessions, browsing full-text results, and inserting cross-session references into the composer.
...
- [DSH Better Reasoning Effort](https://github.com/HaoyueQin/dsh-better-reasoning-effort) - **30 stars** | `MIT`. Set reasoning effort levels for third-party models in the DSH Models page.
...
- [DSH Minigames](https://github.com/lhh010/dsh-minigames) - **30 stars** | `BSD-3-Clause`. A DSH Web bundle that adds a floating mini-games panel without changing the core conversation flow.
...
- [LoopX DSH](https://github.com/loopx-project/loopx) - **5.3k stars** | `Apache-2.0`. A DSH plugin for bootstrapping LoopX, running governed same-session workflows, and displaying a local GoalBar.
...
- [DSH Infinite Gen 4](https://github.com/Minglink/dsh-infinite-gen-4) - **1.4k stars** | `MIT`. A DeepSeek Harness bundle for controlled red-team prompt evaluation with system-prompt injection and a live client status bar.
...
- [OpenWrite DSH](https://github.com/LiPu-jpg/Openwrite) - **718 stars** | `Apache-2.0`. An interactive DeepSeek Harness writing suite for planning, drafting, and revising long-form fiction in a Web profile.
...
- [AgentSight DSH](https://github.com/agentic-os-org/ANOLISA) - **614 stars** | `Apache-2.0`. A DeepSeek Harness observability plugin that records and presents agent activity for inspection.
  - Install: `cd src/agentsight/dsh-plugin && pnpm install && pnpm run build && dsh plugin --profile web add .`

- [Superdesign DSH](https://github.com/superdesigndev/superdesign-skill) - **547 stars** | `MIT`. A DeepSeek Harness bundle that brings Superdesign's code-aware interface design workflow into an agent session.
...
- [DSH Hub Agent Tools](https://github.com/pax-beehive/dsh-hub-cli) - **346 stars** | `MIT`. A DeepSeek Harness bundle that gives agents tools for inspecting, planning, applying, and rolling back reproducible plugin and profile installations.
...
- [DSH Trading Terminal](https://github.com/zhu1090093659/dsh-trading) - **106 stars** | `PolyForm Noncommercial 1.0.0`. An agent-native trading terminal for DeepSeek Harness with market data connectors, research roles, chart context, and approval-gated execution.
...
- [Jev DSH Decision Engine](https://github.com/Devin-AXIS/jev-dsh-decision) - **68 stars** | `No standard license`. A DeepSeek Harness bundle that adds structured decisions for choosing tools, skills, task owners, and output-quality checks.
...
- [DSH Approval Gate](https://github.com/moon09300731/dsh-approval-gate) - **63 stars** | `MIT`. A DSH Web plugin that adds approval gates and configurable permission presets for tool execution.
...
- [DSH Normify](https://github.com/yan-mc/dsh-normify) - **60 stars** | `MIT`. A DeepSeek Harness bundle that enforces architecture rules, bilingual naming, and validation workflows for agent-written projects.
...
- [DSH Council](https://github.com/a1exsun/dsh-council) - **59 stars** | `MIT`. A multi-model deliberation bundle for running bounded councils inside DeepSeek Harness sessions.
  - Install: `npx -p @deepseek-ai/dsh@latest dsh plugin --profile web add @a1exsun/dsh-council@0.1.0`

- [DSH YOLO](https://github.com/hanshanyike/dsh-yolo) - **59 stars** | `MIT`. A DSH assistant that tracks todos, deadlines, milestones, reminders, and cross-session changes in a Web dashboard.
...
- [OpenCode2DSH](https://github.com/FishBottle7/opencode2dsh) - **57 stars** | `MIT`. A DeepSeek Harness bundle that connects OpenCode Zen models through a native llm-pi-ai adapter without an API key.
...
- [DSH Novel Writer](https://github.com/akira399/dsh-novel-writer) - **53 stars** | `MIT`. A DeepSeek Harness writing suite for planning, drafting, and revising novels with a Web sidebar and project workflow.
...
- [Unity DSH Plugin](https://github.com/opdsh/unity-plugin) - **42 stars** | `MIT`. A DeepSeek Harness bundle for controlling the Unity Editor, creating scenes, attaching scripts, importing assets, and running builds.
  - Install: `dsh plugin --profile <name> add @opdsh/unity-plugin`

- [DSH Rewind](https://github.com/SiriLee/dsh-rewind) - **41 stars** | `MIT`. A DeepSeek Harness bundle for rewinding conversation turns and restoring the associated workspace changes.
...
- [DSH Agency Agents](https://github.com/MichengAI/dsh-agency-agents) - **38 stars** | `Apache-2.0`. A DeepSeek Harness bundle with 271 summonable specialist agents for research, engineering, writing, and other tasks.
  - Install: `dsh plugin --profile web add @michengai/dsh-agency-agents@latest --registry=https://registry.npmjs.org/`

- [DSH Doublecheck](https://github.com/PerryLink/dsh-doublecheck) - **38 stars** | `Apache-2.0`. A DeepSeek Harness bundle that adds requirement review, test-evidence gates, adversarial delivery review, and durable handoff reports.
...
- [DSH WorkBuddy Connect](https://github.com/corrinehu/dsh-workbuddy-connect) - **37 stars** | `MIT`. A DeepSeek Harness provider plugin that connects the models available in the WorkBuddy desktop app without separate model configuration.
...
- [DSH LikeTavern](https://github.com/Amakurai/dsh-liketavern) - **36 stars** | `MIT`. A DeepSeek Harness roleplay frontend with character cards, prompt presets, lorebooks, personas, long-term memory, and rollback support.
...
- [Marketing Mindset DSH](https://github.com/axelfreeman/marketing-mindset) - **36 stars** | `MIT`. A DSH bundle that adds the Marketing Mindset skill and workflow guidance to a Harness profile.
  - Install: `dsh plugin --profile <name> add github:axelfreeman/marketing-mindset`

- [WorkBuddy DSH](https://github.com/Axiaohungry/dsh-llm-workbuddy) - **36 stars** | `MIT`. A DSH Web plugin that connects WorkBuddy model providers and account management to the profile settings interface.
  - Install: `dsh plugin --profile web add @axiaohungry/dsh-llm-workbuddy@latest`

- [DSH Data Quality](https://github.com/PerryLink/dsh-data-quality) - **35 stars** | `Apache-2.0`. A DeepSeek Harness bundle for profiling, cleaning, and verifying CSV, TSV, JSON, and JSONL datasets with stored reports.
...
- [DSH Plugin Kit](https://github.com/hyzyn/dsh-plugin-kit) - **35 stars** | `Apache-2.0`. Install a bundle of DSH tools for profiles, prompts, MCP settings, search, and more.
...
- [DSH Persistent Agent Team](https://github.com/wowyuarm/dsh-agent-team) - **34 stars** | `MIT`. A DeepSeek Harness Web bundle for persistent agent teams with member memory, workspaces, channels, and task threads.
  - Install: `dsh plugin --profile web add @wowyuarm/dsh-agent-team`

- [DSH Trading](https://github.com/maddogfinance/dsh-trading) - **34 stars** | `MIT`. A DeepSeek Harness trading bundle with market data, analysis tools, risk guards, verdicts, and Web chart support.
...
- [DSH Thinking Effort](https://github.com/hytime/dsh-thinking-effort) - **32 stars** | `MIT`. A DeepSeek Harness bundle that adds reasoning-effort settings for models and default effort controls for subagents.
  - Install: `dsh plugin --profile web add @hytime/dsh-thinking-effort`

- [DSH WorkBuddy Connector](https://github.com/dingminhua/dsh-connect-workbuddy) - **32 stars** | `MIT`. A DeepSeek Harness bundle that connects signed-in WorkBuddy accounts as selectable model providers and shows read-only account status.
  - Install: `dsh plugin --profile web add dsh-connect-workbuddy`

- [DSH AGY](https://github.com/chaos-03x/dsh-agy) - **31 stars** | `MIT`. A DeepSeek Harness provider plugin for Google Antigravity OAuth login, model access, and multi-account rotation.
  - Install: `dsh plugin --profile web add dsh-agy`

- [DSH MCP Connector](https://github.com/duhu2000/dsh-mcp-connector) - **31 stars** | `MIT`. Manage MCP server connections and search tools across active connections in DSH.
  - Install: `dsh plugin --profile web add dsh-mcp-connector`

- [DSH Recall Plugin](https://github.com/limbo947/dsh-recall-plugin) - **31 stars** | `MIT`. A DeepSeek Harness bundle that returns a conversation to the state captured when a message was sent.
...
- [fnOS DSH Apps](https://github.com/tnnevol/fn-os-apps) - **31 stars** | `AGPL-3.0-only`. A suite of fnOS DSH bundles for CodeBuddy, Codex authentication, and fnOS integration in Web profiles.
...
- [Amadeus for DSH](https://github.com/yyxcnasd/amadeus-for-dsh) - **30 stars** | `MIT`. A DeepSeek Harness Web bundle that adds an Amadeus assistant modeled on Makise Kurisu from Steins;Gate 0.
